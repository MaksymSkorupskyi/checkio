...
def create_intervals(data: Set[int]) -> List[Tuple[int, int]]:
    """ Create a list of intervals out of set of ints. """
    return list(zip(sorted(element for element in data if element - 1 not in data),
                    sorted(element for element in data if element + 1 not in data)))
...
if __name__ == '__main__':
    ...
    print(create_intervals({1, 2, 3, 4, 5, 7, 8, 12}))
    print(create_intervals({-1, 0, 1, 12}))
    ...
    assert create_intervals({-1, 0, 1, 12}) == [(-1, 1), (12, 12)]
