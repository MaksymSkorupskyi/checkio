...
are kept in order and the principal operations on the collection are the addition of entities
to the rear terminal position (enqueue or push), and removal of entities from the front terminal position
(dequeue or pop).
...
You should process all commands and assemble letters which remain in the queue in one word
from the front to the rear of the queue.
...
or events are stored and held to be processed later.
...
def letter_queue(commands: List[str]) -> str:
    """ Return the queue remaining as a string from a sequence of commands as a list of strings."""
    queue = ''
    for command in commands:
        if command.startswith('PUSH'):
            queue += command[5:]
        elif command == 'POP' and queue:
            queue = queue[1:]
    return queue
