...
# v1 - split() and Counter()
def popular_words(text: str,
                  search_words: list) -> dict:
    """
    Return the dictionary where the search words are the keys
    and values are the number of times when those words are occurring in a given text.
    """
    counter = Counter(text.lower().split())
    return {k: counter[k] for k in search_words}
...
def popular_words(text: str,
    ...
    return {word: words.count(word) for word in search_words}


# v3 - split() and count() one-liner
def popular_words(text: str,
                  search_words: list) -> dict:
    """
    Return the dictionary where the search words are the keys
    and values are the number of times when those words are occurring in a given text.
    """
    return {word: text.lower().split().count(word) for word in search_words}
